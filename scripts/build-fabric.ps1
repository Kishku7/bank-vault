...
foreach($v in $Versions){
  ...
  Write-Host "=== BV Fabric $v (mc=$($m.mc)) ==="
  ...
  $jar=Get-ChildItem (Join-Path $fabric "build\libs") -Filter "bank-vault-*.jar"|?{$_.Name -notmatch 'sources'}|Sort-Object LastWriteTime|Select-Object -Last 1
  Copy-Item $jar.FullName (Join-Path $dist ("bank-vault-{0}+{1}-fabric.jar" -f $modver,$v)) -Force
  Write-Host "  -> $v done"
}
