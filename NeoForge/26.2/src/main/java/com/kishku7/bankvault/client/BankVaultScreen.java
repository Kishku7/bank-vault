...
public class BankVaultScreen extends AbstractContainerScreen<BankVaultMenu> {
    ...
    protected void containerTick() {
        ...
        if (++shRefreshTicks >= 60) {   // every 3s
            shRefreshTicks = 0;
            ClientPacketDistributor.sendToServer(new ShareActionPayload(ShareActionPayload.REFRESH, "", 0));
        }
    }
    ...
    private void sendUiState() {
        ...
        ClientPacketDistributor.sendToServer(new UiStatePayload(selectedKey, selectedKey, sv, ""));
    }
    ...
    public boolean keyPressed(KeyEvent event) {
        if (shInputActive) {
            ...
            if (event.isConfirmation()) {
                ...
                shInputActive = false;
                if (!name.isEmpty())
                    ClientPacketDistributor.sendToServer(new ShareActionPayload(ShareActionPayload.INVITE, name, 1));
                return true;
            }
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean doubleClick) {
        ...
        if (ctrlVisible && inside(mx, my, titlesX, ctrlY, secBoxX + secBoxW - titlesX, sbH)) {
            ...
            ClientPacketDistributor.sendToServer(new UiStatePayload(selectedKey == null ? "" : selectedKey, "", "",
                    showSections ? "on" : "off"));
            rebuild();
            return true;
        }
        ...
        if (button == 1 && inside(mx, my, upgX, upgY, upgSize, upgSize) && permLevel >= 3
                && this.menu.getCarried().isEmpty()) { ClientPacketDistributor.sendToServer(new UpgradePayload(false)); return true; }
        ...
        if (button == 0 && permLevel >= 1 && this.menu.getCarried().isEmpty()) {
            if (inside(mx, my, depInvX, depY, depInvW, 11)) {
                ClientPacketDistributor.sendToServer(new com.kishku7.bankvault.net.DepositAllPayload(false)); return true;
            }
            if (inside(mx, my, depAllX, depY, depAllW, 11)) {
                ClientPacketDistributor.sendToServer(new com.kishku7.bankvault.net.DepositAllPayload(true)); return true;
            }
        }
        ...
        if (shTop > 0 && button == 0) {
            ...
            if (inside(mx, my, shBtn1X, shBtnY, shBtn1W, SH_BTN_H)) {
                ...
                else if (inGroup) ClientPacketDistributor.sendToServer(new ShareActionPayload(ShareActionPayload.LEAVE, "", 0));
                return true;
            }
            if (inside(mx, my, shBtn2X, shBtnY, shBtn2W, SH_BTN_H)) {
                if (inGroup) {
                    if (canInvite) ClientPacketDistributor.sendToServer(new ShareActionPayload(ShareActionPayload.LEAVE, "", 0));
                } else if (hasInvite) {
                    ...
                    this.onClose();
                    if (mc != null) {
                        mc.setScreenAndShow(new net.minecraft.client.gui.screens.ConfirmScreen(yes -> {
                            if (yes) ClientPacketDistributor.sendToServer(new ShareActionPayload(ShareActionPayload.ACCEPT, chosen.from(), 0));
            ...
            if (shMgmtMode == 1 && shSelected != null) {
                if (inside(mx, my, shMg1X, shMgmtY, shMg1W, SH_BTN_H)) {
                    ClientPacketDistributor.sendToServer(new ShareActionPayload(ShareActionPayload.KICK, shSelected, 0));
                    shSelected = null; return true;
                }
                if (inside(mx, my, shMg2X, shMgmtY, shMg2W, SH_BTN_H)) {
                    ClientPacketDistributor.sendToServer(new ShareActionPayload(ShareActionPayload.LEVEL_UP, shSelected, 0)); return true;
                }
                if (inside(mx, my, shMg3X, shMgmtY, shMg3W, SH_BTN_H)) {
                    ClientPacketDistributor.sendToServer(new ShareActionPayload(ShareActionPayload.LEVEL_DOWN, shSelected, 0)); return true;
                }
                ...
                shSelInvite = -1;
                ClientPacketDistributor.sendToServer(new ShareActionPayload(ShareActionPayload.DECLINE, from, 0)); return true;
            }
    ...
    private void sendGridView() {
        ...
        ClientPacketDistributor.sendToServer(new GridViewPayload(selectedKey == null ? "" : selectedKey, keys));
    }
